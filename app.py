# app.py — Real provider call only. No fallbacks. Returns HTTP 200 with {ok,reply|error}.
import os, time
...
MODEL_NAME     = os.getenv("MODEL_NAME", "llama3-8b-8192")  # groq default: llama3-8b-8192
SHARED_SECRET  = os.getenv("SHARED_SECRET", "")
ATTEMPT_TIMEOUT_SECS = float(os.getenv("ATTEMPT_TIMEOUT_SECS", "3.0"))  # per HTTP attempt
REQ_TIMEOUT_SECS     = float(os.getenv("REQ_TIMEOUT_SECS", "30"))       # total budget
...
async def call_provider(prompt: str) -> Tuple[bool, str, str, int]:
...
async def diag():
    base, key = _base_url_and_key()
    return {"ok": True, "provider": PROVIDER, "model": MODEL_NAME, "has_key": bool(key), "base": base}
...
async def chat(body: ChatIn, x_shared_secret: str = Header(default="")):
    if not SHARED_SECRET or x_shared_secret != SHARED_SECRET:
        # keep 401 here to surface secret mismatch explicitly
    ...
    ok, text, reason, status = await call_provider(prompt)
    if ok:
        return ChatOut(ok=True, reply=text)

    # compact diagnostic line for Render logs
    print(f"[chat] provider={PROVIDER} status={status} reason={reason}", flush=True)

    # return HTTP 200 with explicit error for Roblox to map (only secret uses 401)
