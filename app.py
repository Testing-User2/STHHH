...
# ── Env (only these two required) ─────────────────────────────────────────────
SHARED_SECRET   = (os.getenv("SHARED_SECRET", "") or "").strip()
OPENAI_API_KEY  = (os.getenv("OPENAI_API_KEY", "") or "").strip()

# Optional; leave empty unless using a proxy/Azure
OPENAI_BASE_URL = (os.getenv("OPENAI_BASE_URL", "") or "").strip() or None
OPENAI_ORG_ID   = (os.getenv("OPENAI_ORG_ID", "") or "").strip() or None
OPENAI_PROJECT  = (os.getenv("OPENAI_PROJECT_ID", "") or "").strip() or None
...
if not OPENAI_API_KEY or not OPENAI_API_KEY.startswith("sk-"):
    raise RuntimeError("OPENAI_API_KEY missing or invalid format")
...
# ── Upstream call with robust error mapping ──────────────────────────────────
...
async def _openai_call(prompt: str) -> Tuple[bool, str, str]:
    ...
    while True:
        ...
        try:
            ...
            body_txt = ""
            try:
                body_txt = getattr(e.response, "text", "") or str(e)
            except Exception:
                body_txt = str(e)
            log.error("[upstream openai] status=%s body=%s", status, body_txt)
